...
def start_scraping(request):
    ...
    if request.method == 'POST':
        ...
        def push_live_status(payload: dict) -> None:
            """Mirror crawler stdout updates into the UI status panel."""
            _update_current_status(
                request,
                _build_status(
                    str(payload.get("state") or "Running"),
                    payload.get("website") or start_url,
                    pages_crawled=payload.get("pages_crawled"),
                    downloaded=payload.get("downloaded"),
                    message=payload.get("message") or payload.get("current_url") or "",
                ),
            )
        ...
        downloaded_documents, crawl_metadata = crawl_and_download(
            ...
            max_pages=max_pages,
            max_pdfs=max_pdfs,
            status_callback=push_live_status,
        )
